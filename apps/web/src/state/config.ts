...
export const DEFAULT_CONFIG: AppConfig = {
  ...
  // Telemetry defaults to OFF. Fresh installs do not share usage data
  // until the user opts in under Settings → Privacy. The analytics gate
  // at `daemon/src/analytics.ts` (`if (telemetry?.metrics !== true) return`)
  // treats anything other than an explicit `true` as off.
...
export function mergeDaemonConfig(
  ...
  // Only mint an anonymous id when reporting is actually on. Fresh
  // installs default to Don't share (metrics === false, no id). An
  // install that somehow has metrics on but no id still gets one so
  // Settings → Privacy does not render that state as "Opted out".
  if (next.telemetry?.metrics === true && !next.installationId) {
